...
void do_ui_stuff()
{
	...
	{
		// TODO - cool hovering and following animation
		// :inventory UI
		...
		if (world->inventory_alpha_target == 1.0 /*world->inventory_alpha != 0.0*/)
		{
			...
			float x_start_pos = ((screen_width - hotbar_width) / 2.0) + (icon_object * 0.5);
		...
		// :building UI
		{
			...
			if (world->building_alpha_target == 1.0 /*world->building_alpha != 0.0*/)
			{
				...
				float x_start_pos = (screen_width - hotbar_size) * 0.5;
				...
				for (BuildingID i = 1; i < BUILDING_MAX; i++)
				{
					...
					Draw_Quad *quad = draw_image_xform(icon->image, xform, get_sprite_size(get_sprite(building->icon)), COLOR_WHITE);
...
int entry(int argc, char **argv)
{
	...
	{
		// buildings[0] = (BuildingData) { .to_build=ARCH_nil, .icon=SPRITE_nil };
	...
	while (!window.should_close)
	{
		...
		// @ship debug change this to pause or to go back to UX_nil
